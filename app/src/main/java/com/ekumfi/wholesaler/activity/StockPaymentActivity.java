...
import com.ekumfi.wholesaler.adapter.PaymentsAdapter;
...
public class StockPaymentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stock_payment);
...
//        menu = findViewById(R.id.menu);
    ...
    protected void onResume() {
        ...
        JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(
                Request.Method.GET,
                API_URL + "stock-payments",
                null,
                response -> {
                    if (response != null) {
                        Realm.init(getApplicationContext());
                        Realm.getInstance(RealmUtility.getDefaultConfig(StockPaymentActivity.this)).executeTransaction(realm -> {
...
//                    realmPayment.setAttended(attended);
    ...
    private static ArrayList<RealmPayment> filter(ArrayList<RealmPayment> models, String search_txt) {
        ...
        for (RealmPayment model : models) {
            ...
            if (
                    model.getShop_name().toLowerCase().contains(search_txt.toLowerCase()) ||
                    model.getPrimary_contact().toLowerCase().contains(search_txt.toLowerCase()) ||
                    model.getOrder_id().toLowerCase().contains(search_txt.toLowerCase())
            ) {
