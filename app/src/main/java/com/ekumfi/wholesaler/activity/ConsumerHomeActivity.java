...
import com.ekumfi.wholesaler.fragment.CartListFragment;
import com.ekumfi.wholesaler.fragment.ChatIndexFragment;
import com.ekumfi.wholesaler.fragment.ConsumerSettingsFragment;
import com.ekumfi.wholesaler.fragment.ProductsFragment;
import com.ekumfi.wholesaler.fragment.SearchProductFragment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(ConsumerHomeActivity.this)).executeTransaction(realm -> {
            RealmResults<RealmCart> realmCarts = realm.where(RealmCart.class)
                    .distinct("seller_id")
                    .equalTo("consumer_id", PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("com.ekumfi.wholesaler" + "CONSUMERID", ""))
                    .findAll();
            for (RealmCart realmCart : realmCarts) {
                FirebaseMessaging.getInstance().subscribeToTopic(realmCart.getSeller_id() + PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("com.ekumfi.wholesaler" + "CONSUMERID", ""))
                        .addOnCompleteListener(task -> {
                            String msg = "successfully subscribed";
                            if (!task.isSuccessful()) {
            ...
            case 1815:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                        if (data != null) {
                            ProductsFragment.productMaterialDialog.setProduct_id(data.getStringExtra("PRODUCT_ID"));

                            ProductsFragment.productMaterialDialog.name_edittext.setText(data.getStringExtra("NAME"));
                            ProductsFragment.productMaterialDialog.name_edittext.setError(null);
                        }
                        break;
...
//        realm.createOrUpdateAllFromJson(RealmDrawingCoordinate.class, responseJson.getJSONArray("drawing_coordinates"));
    ...
    public static void retriev_current_registration_token(Context context, String confirmation_token) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(
                ...
                keyConst.API_URL + "update-confirmation-token/" + PreferenceManager.getDefaultSharedPreferences(context).getString("com.ekumfi.wholesaler" + MYUSERID, ""),
                request,
                response -> {
                },
                error -> {
